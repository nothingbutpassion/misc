	...
	__global const float2* map, int map_step, int map_offset)
{
	...
	if (x < dst_cols && y < dst_rows) {
		...
		uchar4 value = (uchar4)(0, 0, 0, 0);
		float2 src_pos = rmat2(map, x, y);

		if (0 <= src_pos.x && src_pos.x <= src_cols - 1 && 0 <= src_pos.y && src_pos.y <= src_rows - 1) {
			...
			value = convert_uchar4_sat(get_bicubic_8uc4(v[0], v[1], v[2], v[3], yR));
		}
		
		wmat8uc4(dst, x, y) = value;
	}
}
*/
__kernel void remap_8UC4_32FC2(
	...
	__global const float2* map, int map_step, int map_offset)
{
	...
	if (x < dst_cols && y < dst_rows) {
		...
		if (0 <= src_pos.x && src_pos.x <= src_cols - 1 && 0 <= src_pos.y && src_pos.y <= src_rows - 1) {
			...
			value = get_bicubic_32fc2(v[0], v[1], v[2], v[3], yR);
		}
