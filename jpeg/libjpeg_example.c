...
GLOBAL(void) write_JPEG_file (const char* rawFile, int width, int height, int bpp, int colorSpace, const char* jpegFile, int quality)
{
   ...
   */
  if ((infile = fopen(rawFile, "rb")) == NULL) {
		fprintf(stderr, "can't open %s\n", rawFile);
		exit(1);
  }
  if ((outfile = fopen(jpegFile, "wb")) == NULL) {
		fprintf(stderr, "can't open %s\n", rawFile);
		exit(1);
  }
   ...
   */
  row_stride = width * bpp; /* JSAMPLEs per row in image_buffer */ 
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
     ...
     */
    fread(row_pointer[0], row_stride, 1, infile);
...
void dumpDecompress(struct jpeg_decompress_struct* cinfo)
{ 
 ...
 */
GLOBAL(int) read_JPEG_file(const char* jpegFile, const char* rawFile)
{
	...
	*/
	...
	/* More stuff */
	FILE * infile;                /* jpeg file */
	FILE * outfile;               /* raw file */
	...
	*/
	...
	if ((outfile = fopen(rawFile, "wb")) == NULL) {
		fprintf(stderr, "can't open %s\n", rawFile);
		return 0;
	}
	...
	*/
	...
	// Here, we dump some fields of cinfo
	dumpDecompress(&cinfo);
...
{ 
 ...
 int main(int argc, char* argv[]) {
 	...
 	if (argc == 4 && argv[1][0] =='d') {
 		read_JPEG_file(argv[2], argv[3]);
 	} else if (argc == 8 && argv[1][0] =='c') {
 		write_JPEG_file(argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]), argv[7], 100);
 	} else {
 		showUsage(argv[0]);
 	}
 	return 0;
